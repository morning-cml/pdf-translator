...
%PY% -c "import pdfplumber, reportlab, pypdf, requests, tkinter" >nul 2>nul
if errorlevel 1 (
  echo.
  echo    首次运行，正在自动安装所需组件，请稍候几分钟……
  echo.
  %PY% -m pip install pdfplumber reportlab pypdf requests pymupdf fonttools rapidocr-onnxruntime
  if errorlevel 1 (
    echo.
