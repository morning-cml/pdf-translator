...
模型文件（实际 72MB）放用户数据目录 `models/doclayout_yolo_docstructbench_
imgsz1024.onnx` 即自动启用；没有该文件则完全不参与（零行为差异）。下载
（国内镜像）：https://hf-mirror.com/wybxc/DocLayout-YOLO-DocStructBench-onnx
推理用 onnxruntime（随 OCR 组件已装），无需 torch / 显卡。

⚠️ 现状（2026-07-20）：该模型体积大且为 AGPL，**既不入 git 仓库、也不入
打包 exe**，因此普通用户/克隆者默认没有它——本插件对他们等于关闭，属**开发
机上的调试增强**。这是有意的搁置决定（评估见 docs/项目总览.md「版面模型的
处置」），不是遗漏。缺失时静默退回启发式，核心功能不受影响。若未来要面向
用户启用，需先做「首次使用自动下载 + 校验」；若决定砍掉，可连带移除一个
AGPL 依赖。
"""
